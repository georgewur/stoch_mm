...
With the `COVxy` you can run an animation showing how the plume is ditributed in the domain during time.


# Read the data
The `read_stoch_MT3DMS.R` script should be run to determine the central mass moments for all the stochastic MT3DMS

```{r}
rm(list = ls())  # clean the memory
library(gifski) #package required for animating the displacement of the plumes
mm_data = read.csv(file = "stoch_MM_data.csv")
```
...
plotMM_mean_sd = function(mm)
{
  ...
  plt.range = c(min(mean)-max(sd),max(mean)+max(sd),
                main = paste("Central mass moment", mm))
...
col.set = rainbow(n=10)

###just for checking the colors
plot(1,1,type = "p", xlim = c(0,10),ylim = c(0,10),
...
{ 
  plot(1,1, type = "n", xlim=c(0,40),ylim=c(0,20),xlab= "distance (m)",ylab = "height (m)")
  ...
  for (p in 1: length(cov.index))
  {
    ...
    points(M1x[i],M1y[i],pch=20,col=col.set[p],cex=2)
    #polygon(ellipse(M,scale=c(M2x[i],M2y[i]),centre=c(M1x[i],M1y[i]),level=0.68,fill=TRUE),col=rgb(0,0,1,0.2),border=NA)
    polygon(ellipse(M,scale=0.1*c(M2x[i],M2y[i]),centre=c(M1x[i],M1y[i]),level=0.68,fill=TRUE),col=col.set[p],border=NA)
    grid(col="black")
  }
}
 par = old.par
