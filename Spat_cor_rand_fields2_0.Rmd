...
With the function below you can set the seed (a random number) so you can can reproduce exactly the samen k-fields
...
xy <- expand.grid(1:num_row, 1:num_col) #to set up the grid 
names(xy) <- c("x","y")
```
...
The real mean and standard deviation need to be scaled since the `ln(k)` is normal distribution resulting in a lognormal `k` distribution to be used in MODFLOW.
...
range = 1 #low value means hardly any correlation length
nmax = 1 # low values with low range means very random simulation
...
spplot(k_fields)
hist(k_fields$sim1)
cat('scaled mean log(k): ',mean(k_fields$sim1), 'scaled standard deviation log(k) : ', sd(k_fields$sim1))
hist(exp(k_fields$sim1))
cat('simulated k-field mean :',mean(exp(k_fields$sim1)), 'and standard deviation :', sd(exp(k_fields$sim1)))
#real_k_field =
#spplot(k_fields,formula= exp(sim1)~x+y)
spplot(k_fields,formula= exp(z)~x+y|name)
```
...
write.table(data_kvalue,file = paste0("k_col",num_col,"_row",num_row,"k",k_mean,"sd_k",k_sd,"simnr",nsim,".txt"), 
